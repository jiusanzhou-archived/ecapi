...
<?php echo $this->smarty_insert_scripts(array('files'=>'common.js,user.js,plupload.full.min.js,plupload_zh_CN.js,qiniu.min.js')); ?>
</head>
<body>
...
?>
     ...
     <form name="formIdcard" action="user.php" method="post" onSubmit="return true" >
     <table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#dddddd">
        <tr>
          <td width="28%" align="right" bgcolor="#FFFFFF"><?php echo $this->_var['lang']['id_number']; ?>：</td>
          <td width="76%" align="left" bgcolor="#FFFFFF"><input required name="id_number" type="" size="18"  class="inputBg" /></td>
        </tr>
        <tr>
          <td width="28%" align="right" bgcolor="#FFFFFF"><?php echo $this->_var['lang']['id_card_z']; ?>：</td>
          <td align="left" bgcolor="#eee" height="100px">
            <input name="id_card_z" id="id_card_z_url" class="inputBg" hidden />
            <a href="#" id="pickimg_id_card_z">选择正面图片</a>
          </td>
        </tr>
        <tr>
          <td width="28%" align="right" bgcolor="#FFFFFF"><?php echo $this->_var['lang']['id_card_f']; ?>：</td>
          <td align="left" bgcolor="#e2e2e2" height="100px">
            <input name="id_card_f" id="id_card_f_url"  class="inputBg" hidden />
            <a href="#" id="pickimg_id_card_f">选择反面图片</a>
          </td>
        </tr>
        <tr>
          <td width="28%" align="right" bgcolor="#FFFFFF"><?php echo $this->_var['lang']['id_result']; ?>：</td>
          <td align="left" bgcolor="#FFFFFF"><input name="id_result" type="" size=""  class="inputBg" /></td>
        </tr>
        <tr>
          <td colspan="2" align="center" bgcolor="#FFFFFF"><input name="act" type="hidden" value="act_edit_id_card" />
...
<script type="text/javascript">
  ...
  var option_id_card_z = {},
      option_id_card_f = {};
  var option_id_card = {
    runtimes: 'html5,flash,html4',    //上传模式,依次退化
    //uptoken_url: 'api/?token',
    uptoken: 'ruHzZZpeucn3nFyhuKTBfKrL3JEdkZ5-_VBa-Cqb:IDoEjWIQ6UQ0O7vf5B1MbsSCh7U=:eyJzY29wZSI6Inh1YW53dSIsImRlYWRsaW5lIjoxNDUyMTUxMzM0fQ==',
    unique_names: true,
    save_key: false,
    domain: 'http://7xpe4q.com1.z0.glb.clouddn.com/',
    max_file_size: '2mb',
  ...
  for (var i in option_id_card) {
    option_id_card_z[i] = option_id_card[i];
    option_id_card_f[i] = option_id_card[i];
  };
  option_id_card_z['browse_button'] = 'pickimg_id_card_z';
  option_id_card_z['init'] = {
        ...
        'FileUploaded': function(up, file, info) {
              var sourceLink = up.getOption('domain') + JSON.parse(info).key;
              console.log(sourceLink);
              document.getElementById('id_card_z_url').setAttribute('value', sourceLink);
              document.getElementById('pickimg_id_card_z').innerHTML = file.name;
        },
  ...
  option_id_card_f['browse_button'] = 'pickimg_id_card_f';
  option_id_card_f['init'] = {
        'FilesAdded': function(up, files) {
            plupload.each(files, function(file) {
                // 文件添加进队列后,处理相关的事情
            });
        ...
        'FileUploaded': function(up, file, info) {
              var sourceLink = up.getOption('domain') + JSON.parse(info).key;
              document.getElementById('id_card_f_url').setAttribute('value', sourceLink);
              document.getElementById('pickimg_id_card_f').innerHTML = file.name;
        },
        'Error': function(up, err, errTip) {
               //上传出错时,处理相关的事情
        },
  ...
  var uploader_z = Qiniu1.uploader(option_id_card_z);
  var uploader_f = Qiniu2.uploader(option_id_card_f);
</script>
<script type="text/javascript">
